...
function renderFerretero(){
  ...
  html += `<div class="card"><h2>Avance general vs meta</h2>`;
  ETAPAS_SFOR.forEach(e => {
    const v = f.etapas[e] || 0;
    const p = pct(v, f.meta);
    html += `<div class="row" style="margin-top:6px"><span class="sub">${ETAPAS_SFOR_LABEL[e]}</span><span class="sub" style="color:${colorPct(p)}">${p}%</span></div>${barra(p)}`;
  });
  html += `</div>`;
...
function renderResumen(){
  ...
  html += `<div class="card">
    ...
    <div class="sub">${albRezago} en rezago &middot; ${albHoy} de hoy</div>
    ${barra(albAvance)}
  </div>`;

  html += `<div class="card">
    <div class="row"><span class="label">Trailers / tortones</span><span class="big" style="font-size:18px">${unidadesActivas}</span></div>
    <div class="sub">unidades activas en este momento</div>
  </div>`;
  ...
  html += `<div class="card">
    <div class="row"><span class="label">Rutas ferreteras</span><span class="big" style="font-size:18px;color:${colorPct(fPct)}">${fPct}%</span></div>
    <div class="sub">${estado.ferretero.etapas.cerradas} cerradas de ${estado.ferretero.meta}</div>
    ${barra(fPct)}
  </div>`;

  document.getElementById("view-resumen").innerHTML = html;
  document.getElementById("turnoTitle").textContent = "Tablero — " + estado.turno;
